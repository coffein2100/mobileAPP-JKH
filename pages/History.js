...
const masChet =[
  /* {id:1, nameRooms:"test", nameChetchik:'gaz', date: '12-05-2023', readmeter: "5", flow: '0' },
    ...
    const [historyList, sethistory] = useState(masChet
        /* {id:1, nameRooms:"test", nameChetchik:'gaz', date: '12-05-2023', readmeter: "5", flow: '0' },
        {id:2, nameRooms:"test", nameChetchik:'electro', date: '12-06-2023', readmeter: "10", flow: '5' },
        {id:3, nameRooms:"test", nameChetchik:'electro', date: '12-09-2023', readmeter: "30", flow: '20' },
        {id:4, nameRooms:"test2", nameChetchik:'water', date: '12-05-2023', readmeter: "15", flow: '10' },
...
const addHistory = (history) => {
      ...
      /*   let last_element = historyList.findLast(readmeter => readmeter && readmeter.nameRooms === history.nameRooms && readmeter.nameChetchik ===history.nameChetchik ); */
        
/* console.log(last_element);  */


/*  let lastCount = historyList.reduce(function(prev, current, history) {
  
    if (current.readmeter > prev.readmeter && current.nameRooms == history.nameRooms && current.nameChetchik == history.nameChetchik ) {
        return current;
 ...
 let newFlow= history.readmeter - lastCount.readmeter;

 if (Number.isNaN(newFlow)) {newFlow=0}; 

console.log(newFlow);   */

let result = historyList.find(({ readmeter, nameRooms,nameChetchik}) => Math.max(readmeter) && nameRooms == history.nameRooms && nameChetchik == history.nameChetchik);

console.log(result);
if (result === undefined) {result=0;newFlow=0} else{

newFlow= history.readmeter - result.readmeter;};

        sethistory((list) => {
            history.id = Math.random().toString();
            history.date = inputDate.toLocaleDateString("ru-RU");
              history.flow= newFlow;   
          return [
            history,
            ...list
          ]
